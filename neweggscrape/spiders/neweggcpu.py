...
from ..items import AresscrapeCPU, Review
...
class NeweggCPUSpider(Spider):
    ...
    def boardproductpage(self, response):
        ...
        if ('Processors Type' in itemdict and itemdict['Processors Type'] == 'Desktop') or 'Brand' not in itemdict or 'CPU Socket Type' not in itemdict:
            yield None
        else:
            item['brand'] = itemdict['Brand'].strip()
            item['model'] = itemdict['Name'].strip()
            ...
            for r in rev:
                ...
                re_item = Review()
                print('consssssssssssssssssss', cons)
                re_item['cons'] = cons
                re_item['pros'] = pros
