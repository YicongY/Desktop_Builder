...
from ..items import Aresscraperam,Review
...
class NeweggRamSpider(Spider):
    ...
    start_urls = [
        'http://www.newegg.com/Desktop-Memory/SubCategory/ID-147/Page-%s?/Pagesize=90&Order=BESTSELLING'
    ...
    def boardproductpage(self, response):
        ...
        if 'Capacity' not in itemdict:
            ...
            if 'GB' in itemdict['Capacity']:
                item['capacity'] = itemdict['Capacity'].split('GB')[0]
            if 'Speed' in itemdict:
                if 'DDR4' in itemdict['Speed']:

                    item['speed'] = itemdict['Speed'].split('DDR4')[1].split(' ')[1].strip()
                    item['ram_type'] = 'DDR4'

                elif 'DDR3' in itemdict['Speed']:
                    item['speed'] = itemdict['Speed'].split('DDR3')[1].split(' ')[1].strip()
                    item['ram_type'] = 'DDR3'
            if 'Color' in itemdict:
                item['color'] = itemdict['Color']
            if 'LED Color' in itemdict:
                item['led'] = 'Yes'
            else:
                item['led'] = 'No'
