...
from ..items import AresscrapeCase, Review
...
class NeweggCaseSpider(Spider):
    ...
    start_urls = [
        'http://www.newegg.com/Desktop-Memory/SubCategory/ID-7/Page-%s?'
        % page for page in range(6, 10)
    ]
    visitedURLs = set()
    ...
    def boardproductpage(self, response):
        ...
        if 'Type' not in itemdict or 'Motherboard Compatibility' not in itemdict:
            yield None
        else:
            item['brand'] = itemdict['Brand']
            item['type'] = itemdict['Type']
            item['compatibility'] = itemdict['Motherboard Compatibility']
